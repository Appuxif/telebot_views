...
async def ensure_subscription(
    sub_chat_id: int, chat_id: int, user_id: int, force: bool = False, business_connection_id: Optional[str] = None
) -> bool:
    ...
    cache_key = f'sub_chat:{sub_chat_id}:user:{user_id}:sub'

    @with_cache(cache_key, 60 * 5, force=force)
    async def _inner() -> dict[str, bool]:
        subscription_result = await check_subscription(sub_chat_id, user_id)
        result = {'subscription_result': subscription_result}
        if not subscription_result:
            chat = await get_chat(sub_chat_id)
